...
def highlight_word_regex(text, word):
    pattern = re.compile(re.escape(word), re.IGNORECASE)
    return pattern.sub(f":red-background[{word}]", text)
...
ids_string = st.text_area(
    label="Input IDs separated by a comma", placeholder="34175218, 33502370"
)
...
if st.session_state.df is not None:
    ...
    st.markdown(
        "Choose a word from the table in the dropdown below, to see the title and abstract of papers that contain that word"
    )
    ...
    if selected_word:
        ...
        articles = [
            find_article(st.session_state.article_dicts, r) for r in related_article_ids
        ]
        st.markdown(
            f" ### 🗃️ Seeing related Titles & Abstracts for: :red[{selected_word}]"
        )
        ...
        for article in articles:
            ...
            with container:
                ...
                # Test the function with the word "app"
                highlighted_text_regex = highlight_word_regex(
                    article["abstract"], selected_word
                )
                st.markdown(highlighted_text_regex)
